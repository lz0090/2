...
# PM_coordinate = np.load('D:/桌面/新仿真记录/PM坐标/PM_40/PM_40_server_4_coverage_30_coordinate.npy')
...
class MyEnv(gym.Env):
    ...
    def step(self, action):
        ...
        server_delay = np.zeros((num_server, 1), dtype=np.float16)  # 创建簇内总时延数组，用于存储每个簇内总时延(最大最小情况下的总时延)
        total_server_delay = np.zeros((num_server, 1), dtype=np.float16)  # 创建簇内总时延数组，用于存储每个簇内总时延(对比方案下的实际总时延)
        ...
        for i in range(num_server):
            ...
            if index:
                ...
                total_server_delay[i] = len(index) * t_auth + t_pre + (0.040125 * len(index) - 0.055875 ) + sum(tmp_delay)


        np.save('./IES_delay_MAXMIN/PM_{}_server_{}_coverage_{}_IES_delay.npy'.format(config.glo_num_PM, config.glo_num_server, config.glo_coverage_server), total_server_delay)
        ...
        if flag_eta == 1 and flag_compu == 0:
            ...
            info = {'correct': [current_max, sum(total_server_delay)]}
...
#                 server_delay[i] += t_auth  # 先将簇i内的认证时间累计求和
#             if index:
#                 server_delay[i] += sum(tmp_delay) + t_pre + (0.040125 * len(index) - 0.055875 ) # 对簇i内的所有设备的时延求和再加上认证时延得到整个簇内的总时延
#
#         np.save('./IES_delay/PM_{}_server_{}_coverage_{}_IES_delay.npy'.format(config.glo_num_PM, config.glo_num_server, config.glo_coverage_server), server_delay)
#
#
#         ###### 计算负载均衡约束
#         sum_differ = 0  # 存储请求量差的平方和
...
#         ###### 判断是否满足负载均衡约束和IES不过载约束
#         if flag_eta == 1 and flag_compu == 0:
#             current_max = sum(server_delay)  # 当前的整个系统的总时延
#
#             # reward = (delay_max / current_max) * 200 * (math.exp(- sum_differ) + 1)
#             reward =  1000 * math.exp( - current_max )    # 改动后的奖励函数
#             # reward =  num_PM * 5 * (delay_max / current_max) ** 2  # 改动后的奖励函数
